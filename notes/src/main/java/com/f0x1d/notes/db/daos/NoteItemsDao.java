...
public interface NoteItemsDao {
    ...
    @Query("SELECT * FROM NoteItem WHERE to_id = :toId order by position asc")
    List<NoteItem> getAllByToId(long toId);
    ...
    @Query("UPDATE noteitem SET text = :text WHERE to_id = :toId AND position = :position")
    void updateElementTextByPos(String text, long toId, int position);
    ...
    @Query("DELETE FROM NoteItem WHERE to_id = :toId")
